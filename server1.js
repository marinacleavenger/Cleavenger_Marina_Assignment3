...
app.post("/login", function (request, response) {
    ...
    let user_reg_data = JSON.parse(raw_user_data);
    ...
    if (typeof user_reg_data[username_entered] !== 'undefined') {
        ...
        if (!isValidPassword(password_entered)) {
            ...
            // response.redirect("./login.html?error=password");
            response.redirect(`./invoice.html?user=${username_entered}`);
            return;
        } else {
            // Incorrect Password
...
//anthonys code for redirecting tot he .js file
